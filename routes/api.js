...
router.get('/menu_create', function(req, res, next) {
  ...
  util.getToken(aotuConfig, function(result) {
    ...
    request.post({
      ...
      //个性化菜单设置
       var form = !!key ? aotuConfig[key] : aotuConfig['menu'];
var url="https://api.weixin.qq.com/cgi-bin/menu/addconditional?access_token=";
var zhanxingshimenu = !!key ? aotuConfig[key] : aotuConfig['rolemenu'];
form.button[0].sub_button.push(zhanxingshimenu.button[0].sub_button[0]);
form.matchrule=zhanxingshimenu.matchrule;
     request.post({
      url: url + access_token,
      form: JSON.stringify(form)
    }, function(error, response, body) {
...
//发送群发消息
router.get('/send_all_text',function(req,res,next){
  debugger;
  var content = "liweiwei auto sent message 3 time";//req.body.msgContent;
...
router.get('/login',function(req,res,next){
  ...
  var url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+aotuConfig.appid+'&redirect_uri='+rUrl+'&response_type=code&scope=snsapi_base&state=123#wechat_redirect';
  res.redirect(url);
});
